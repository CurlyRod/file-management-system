<nav class="pc-sidebar">
    <div class="navbar-wrapper">
        <div class="m-header">
            <a href="#" class="b-brand text-primary">
                <!-- ========   Change your lssogo from here   ============ -->
                <img src="../../images/folder-blue.png" class="img-fluid logo-lg" alt="logo"> 
                <span class="text-white logo-txt fw-bold">Access Grant System</span>
            </a>
        </div>
        <div class="navbar-content">
            <ul class="pc-navbar">
                <li class="pc-item">                 
                </li>

                <li class="pc-item pc-caption">
                    <label>Pages</label>
                    <i class="ti ti-dashboard"></i>
                </li>
                <li class="pc-item">
                    <a href="../../../page/admin/myfiles/" class="pc-link">
                        <img src="../../images/attach.png" alt="">
                        <span class="pc-mtext">My Files</span>
                    </a>
                </li>
                <li class="pc-item">
                    <a href="../../../page/admin/resolved-tickets/" class="pc-link">
                        <img src="../../images/excel.png" alt="">
                        <span class="pc-mtext">Resolved Tickets</span>
                    </a>
                </li> 
                <li class="pc-item">
                    <a href="../../../page/admin/transaction-code/" class="pc-link">
                        <img src="../../images/transaction-history.png" alt="">
                        <span class="pc-mtext">Transaction Code</span>
                    </a>
                </li> 
                <li class="pc-item">
                    <a href="../elements/bc_color.html" class="pc-link">
                        <img src="../../images/request.png" alt="">
                        <span class="pc-mtext">Request</span>
                    </a>
                </li> 
                <li class="pc-item">
                    <a href="../elements/bc_color.html" class="pc-link">
                        <img src="../../images/bar-chart.png" alt="">
                        <span class="pc-mtext">Analytics</span>
                    </a>
                </li>
            </ul>
        </div>
    </div>
</nav>